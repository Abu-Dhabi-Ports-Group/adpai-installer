...
| `CERTIFICATE_VERIFY_FAILED` or `SSLCertVerificationError` during `az extension add` | Corporate TLS inspection, commonly Zscaler, is trusted by Windows but not by Azure CLI's Python certificate bundle. Export the Zscaler root CA as PEM, then run `$env:REQUESTS_CA_BUNDLE="<path-to-pem>"` and `$env:CURL_CA_BUNDLE=$env:REQUESTS_CA_BUNDLE` before rerunning the installer. The Windows installer also tries to auto-export a Zscaler root certificate from the Windows certificate store and retry once. |
